...
;función auxiliar para board-set-play-piece
;Descripción: otra manera de obtener columnas de una manera no recursiva
;Dominio: board X column-position
...
(define (find-zero column)
  ; Función auxiliar para board-set-play-piece para obtener elemento en una posición específica
...
;función auxiliar para board-set-play-piece que busca el 0 más bajo
;Dominio: column X column-position
;Recorrido: number (posicion si encuentra el 0) boolean (falso si no lo encuentra)
...
;función auxiliar para board-set-play-piece que actualiza la fila
;Dominio: current-row X column-position X piece
;Recorrido: Lista (fila actualizada)
...
;Dominio: board (board) X number (column-position) X list (new row)
...
;RF07
;Función auxiliar para board-check-vertica-win
...
(define (column-check-vertical column)
  (cond
    ...
    [(and (not (equal? (first-at-list column) 0))  ;not equal asegura que el primer elemento de la columna no sea 0
     ...
     (if (equal? (first-at-list column) "red") 1 2)]  ; con lo de arriba me aseguro que hay un ganador , con esto veo quien ganó: 1 para rojo, 2 para amariillo
    [else
...
;RFO8
;Función auxiliar para board-check-horizontal-win
...
(define (row-check-horizontal row)
  (cond
    ...
    [(and (not (equal? (first-at-list row) 0))  ; not equal? asegura que el primer elemento de la columna no sea 0
     ...
     (if (equal? (first-at-list row) "red") 1 2)]  ; 1 para rojo, 2 para amarillo
    [else
...
;Dominio: board row col
;Recorrido: red yelllow 0
...
;Descripción : Verificar diagonal descendente desde una posición
;Dominio: board X row X col
;Recorrido: number (0 no diagonal, 1 diagonal rojo, 2 diagonal amarillo)
...
;Descripción : Verificar diagonal ascendente desde una posición
;Dominio: board X row X col
;Recorrido: number (0 no diagonal, 1 diagonal rojo, 2 diagonal amarillo)
...
;Descripción: crea una nueva lista aplicando función f a cada elemento de la lista l
;Dominio: function (f) X list (l)
;Recorrido: list
;Recursión: natural
...
;Descripción: calcula el largo de la lista
;Dominio: lista (lst)
;Recorrido: number (tamaño)
;Recursión: natural
(define (mylength lst)
  (if (null? lst)
      0
